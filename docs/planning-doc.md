...
### Schema for extraction (confirmed) DONE
...
### Schema for formatting (in planning stage) (ETL) DONE
...
3) ~~__PRIORITY__ : Need rate limiters to avoid ip blocking. **Need a way to determine ratio of ~~scraping to rest.**~~ DONE
4) ~~__PRIORITY__ : Need a way to skip scraping on pages where other or other information is shown.~~ DONE
...
__Goal__ : 100000 efficient Scrapes. With minimal errors and no unidentified errors.
__Bounds__ : Possible of 3 billion + books with equivalent or less than that author pages.


## DATA STORAGE (Not Started)
### Data Schema
1) PostGres DB with postgres vector storage for similarity embeddings.
